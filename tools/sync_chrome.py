...
MESH_HTML = '<div class="ambient-mesh" aria-hidden="true"><div class="mesh-orb mesh-orb-1"></div><div class="mesh-orb mesh-orb-2"></div></div>'
...
def nav_html(page, prefix):
    ...
    return (
        ...
        + '<button id="cmd-k-btn" class="cmd-k-trigger" onclick="toggleCmdPalette()" aria-label="Open command palette" title="Search & Shortcuts (⌘K)"><span>⌘K</span></button>'
        + "</nav>"
    )
...
def main():
    ...
    for path in sorted(glob.glob("*.html")):
        ...
        # Static ambient background mesh + skip link + focus target
        if 'class="ambient-mesh"' not in html:
